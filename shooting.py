...
class Shoot():
    ...
    def __init__(self, dataframe: DataFrame):
        ...
        try:
            ...
            raise Exception(f"Shooting Analyser failed: {err.args[0]}")
    ...
    def __update_shot_data(self, cycle: int, player: str, x: float, y: float, players_inside: int, on_target: int):
        ...
        show_time = int(self.__df.loc[cycle, 'show_time'])
        data = {
            'show_time': show_time,
            'player': player,
    ...
    def __check_shot(self, cycle: int):
        ...
        """
        if not self.__df.loc[cycle, 'show_time'] in self.__play_on_cycles:
            return
    ...
    def __check_goal(self, cycle: int):
        ...
        Checks if playmode at index indicates a goal.
            ...
            Parameters:
                    cycle (int): Indicates the index to look for on match DataFrame
        """
        split_play_mode = self.__df.loc[cycle, 'playmode'].rsplit('_')
    ...
    def _analyze(self):
        ...
        """
        filtered_df = self.dataframe[self.dataframe['playmode'] == 'play_on']

        self.__play_on_cycles = list(filtered_df['show_time'])
    ...
    def describe(self):
        ...
        """
        name_l = self.__df.loc[1, 'team_name_l']
        ...
        name_r = self.__df.loc[1, 'team_name_r']
